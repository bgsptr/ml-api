...
# give role
@app.route("/roles/<int:role_id>/<username>", methods=["PUT"])
@cross_origin()
def set_roles(role_id, username):
    cur = mysql.connection.cursor()
...
def edit_del_profile(email):
    if request.method == "PUT":
        ...
        ## improve lagi
        update_data = supabase_client.table('users').update({'fname': fname, 'lname': lname}).execute()
        ...
        return jsonify({"error": "api method bad request"}), 400
